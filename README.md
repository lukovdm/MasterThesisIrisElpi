...
* Application #1, `is_del_list`, can be found in `eIris/experiments/dellists.v`.

**Section 2: Fixpoints in Higher-Order Separation Logic**

* The unary least fixpoint was formalized in Iris and can be found in the Iris repository at [`iris/bi/lib/fixpoint_mono.v`](https://gitlab.mpi-sws.org/iris/iris/-/blob/3da27a78fb7a92dcae79c47a9a64f359858386b6/iris/bi/lib/fixpoint_mono.v).

**Section 3: Data Structure Verification**

* The first example, delete lists with a parameter, `is_list_with_tl`, can be found in `eIris/experiments/dellists.v`.

* The second example, BSTs, `is_search_tree`, can be found in `eIris/experiments/sets.v`.

* The third example, rose trees, with `is_ho_list` and `is_rose_tree`, can be found in `eIris/experiments/rosetrees.v`.

**Section 4: Total Program Correctness**

* The total weakest precondition was formalized in Iris and can be found in the Iris repository at [`iris/program_logic/total_weakestpre.v`](https://gitlab.mpi-sws.org/iris/iris/-/blob/3da27a78fb7a92dcae79c47a9a64f359858386b6/iris/program_logic/total_weakestpre.v).

* The total weakest precondition is reformalized using the `Iris Inductive` command in `eIris/experiments/twp.v`

* Proof of the total correctness of `lookup` Hoare Triple can be found in `eIris/experiments/dellists.v` as the lemma `lookup_spec`.

* The partial weakest precondition was formalized in Iris and can be found in the Iris repository at [`iris/program_logic/weakestpre.v`](https://gitlab.mpi-sws.org/iris/iris/-/blob/3da27a78fb7a92dcae79c47a9a64f359858386b6/iris/program_logic/weakestpre.v).
...
* The definitions `is_list_with_tl_pre` and `is_list_with_tl` are generated by the inductive, `Iris Inductive is_list_with_tl`, and inspected by calling `Print is_list_with_tl_pre` and `Print is_list_with_tl` in `eIris/experiments/dellists.v`.
...
* The proper for `is_list_with_tl` is generated by the inductive, `Iris Inductive is_list_with_tl`, and inspected by calling `About is_list_with_tl_pre_mono` in `eIris/experiments/dellists.v`. Note that this uses the pointwise signature combinator as referenced in footnote 2 of the paper.
...
* The lemma `tac_wand_intro` is part of Iris and can be found in Iris repository at [`iris/proofmode/coq_tactics.v`](https://gitlab.mpi-sws.org/iris/iris/-/blob/3da27a78fb7a92dcae79c47a9a64f359858386b6/iris/proofmode/coq_tactics.v).

* All tactics given in this section: `refine-igoal-with`, `eiIntro-ident`, and `eiIntro-fresh`
  and can be found in `eIris/proofmode/elpi/eiris_tactics.elpi`

* The definition of `pm-reduce` can be found in `eIris/proofmode/elpi/reduction.elpi`
...
The lines of code are generated from `utils/generate_linecount_table.py`. Reimplementing the total weakest precondition in Iris can be found in the next section.

## Setting up the replacing twp experiment
We create this experiment by cloning Iris, adding a patch and copying the eIris folder into the Iris repository. We have added a copy of this structure in the folder `Iris`, but explain below how to replicate it.

1. Clone the [Iris repository](https://gitlab.mpi-sws.org/iris/iris/) at commit `c773500a92ba4fa0ac4213e98ce7b00a8ad99e39`.

2. Apply the patch found in `utils/0001-Fix-iris-for-eIris.patch` using `git apply <path to patch>`.

3. Copy the `eIris` folder to the cloned `Iris` folder.

To run Iris with the new twp, follow these instructions.

1. Follow the Iris dev setup instructions. Including making a new switch and activating it!

2. Run `make` in the Iris directory.
...
- **eIris**: This folder contains reimplemented + Iris Inductive, including Elpi source files.
  + **common**: Contains the Elpi source for the commonly used elpi functions.
    * **utils.v**: Additional functions and datatypes commonly used in other parts of our prototype.
    * **inductive_datatypes.v**: Datatypes necessary for storing properties in the Elpi inductive database.
...
- **utils** Containing python scripts to generate tables
  - **timing**: Contains a Python script and Rocq source file which tests the speed of the intro pattern parser written in Elpi.
