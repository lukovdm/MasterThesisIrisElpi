...
\section{Contexts}\label{sec:iriscontext}
...
\end{elpicode}
Note that references to Coq object cannot be directly written as \elpii{«~\ldots~»}. Thus, the above listing is purely for demonstration purposes. We will discuss how to create Coq objects in \cref{ssec:ceqoute}.
...
Coq function terms work again similarly. Take, for example, the Coq term consisting of a function taking a number and adding one to it, \coqi{fun (n: nat), n + 1}. This is represented in Elpi as follows.
\begin{elpicode}
...
\begin{elpicode}
  FUN = {{ fun (n: nat), n + 1 }}
\end{elpicode}
Now \elpii{FUN} has the following value.
\begin{elpicode}
  fun `n` (global (indt «nat»)) 
            (n \ app [global (indt «sum»), 
                      n, app [global (indc «S»), 
