...
\coqinline{iIntros} consists of a DSL for quickly introducing different logical connectives in \iris. It has also been extended to deal with often seen initial steps in a proof, like \coqinline{simpl}, \coqinline{done} and others. We will show a few examples of how \coqinline{iIntros} can be used to help proof lemmas.
...
\end{coqcode}
It consists of a forall existential, an exists \todoo{same here}, a conjunction and a disjunction. We can again use one application of \coqinline{iIntros} to introduce and eliminate the premise. \coqinline{iIntros "%x [[%y [Hx Hy]] | H0]"} takes the proof to the following state of two goals
\begin{coqcode}
  (1/2)
  P: nat → iProp
  x, y: nat
  ...
  P 1
\end{coqcode}
\todoo{Every intro pattern in the list is a wand introduction or forall introduction, then every intro pattern can be one of the following things, bullet list}
...
\end{figure}
\todoo{add types of arrows, and add introducer and destructor in one box}
...
\end{elpicode}
\todoo{refer to keywords, the kind keyword takes this and does this}
...
\end{elpicode}
\todoo{add whole definition in one diagram and explain all}
...
\begin{elpicode}
  ...
  tokenize S O :- 
    ...
    tokenize.rec SS O.
\end{elpicode}
\todoo{the first line defines the signature of our predicate, i means this, o means this}
This is the entry point of our tokenizer. It takes \elpiinline{string} as input and gives a list of \elpiinline{token} as output. Next we have our first rule, \elpiinline{tokenize S O}. Here \elpiinline{S} is bound to the input \elpiinline{string} and \elpiinline{O} is bound to our output \elpiinline{list token}.
\todoo{this is weird, so explain this more}
A rule now consists of a list of new rules that have to be solved, \elpiinline{rex.split "" S SS} and \elpiinline{tokenize.rec SS O}\footnote{Names in \elpi can have special characters in them like \elpiinline{.} and \elpiinline{-}, thus, \elpiinline{tokenize.rec} is a fully separate predicate.}. Thus basic execution of a program in \elpi consists of a list of rules with their own list of subsequent rules to execute and a stack of predicates to solve. Each time we try to find a rule in the program that can solve our topmost predicate and add any new predicates we need to solve to the top of the stack.
\quest{Should this be here?}
\todoo{Relate more to syntax and example, give type of rex.split and tokenize.rec}
...
\todoo{refer to previous definition but say it is equal and shorter}
\begin{elpicode}
  pred tokenize i:string, o:list token.
...
\todoo{Explain backtracking here}
