...
The pre fixpoint function is defined as \coqi{is_MLL_pre}.
...
\begin{coqcode}
  ...
  Global Instance exists_IProperTop {A} : 
    IProperTop (bi_wand) (@bi_exist A) 
               (fun F => .> bi_wand ==> F).
\end{coqcode}
Thus, when searching for the instance, we also have to fill in the \coqi{A}. The number of arguments we have to fill in when searching for an \coqi{IProperTop} instance differs per connective. We take the following approach.
\begin{elpicode}
