    ...
    \begin{center}
        \begin{tikzpicture}[node distance=1cm and 1cm,scale=0.6, every node/.style={transform shape}]
            ...
            \begin{scope}[local bounding box=bbox]
                ...
                \node[alg={Construct \& prove \\ fixpoint properties}, below=of signproff] (fpp) {};
                \node[obj, ce, below=of fpp] (unfolding) {Unfolding \\ property};
                \node[obj, ce, right=of unfolding] (iter) {Iteration \\ property};

                \node[alg={Construct \& \alert<2>{prove} \\ \alert<2>{constructor lemmas}}, asterix, below=of fixpoint] (genconstrlem) {};
...
\begin{frame}{Conclusion}
    \begin{itemize}[<+->]
        \item Proof search algorithm for monotonicity of pre fixpoint functions
        \item Elpi implementation
    \end{itemize}
\end{frame}

\appendix

\begin{frame}{Evaluation of Elpi}
